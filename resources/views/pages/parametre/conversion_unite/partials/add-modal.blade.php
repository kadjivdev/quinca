...
<div class="modal fade" id="addConversionModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content border-0 shadow">
            ...
            <form id="conversionForm" method="POST" action="{{ route('conversion.store') }}" class="needs-validation" novalidate>
                @csrf
                <div class="modal-body p-4">
                    <div class="row g-4">
                            ...
                            <div class="conversion-units-selector p-3 bg-light rounded">
                                <div class="row align-items-center text-center">
                                    ...
                                    <div class="col">
                                        ...
                                        <select class="form-select" name="unite_source_id" required>
                                    ...
                                    <div class="col">
                                        ...
                                        <select class="form-select" name="unite_dest_id" required>
                                            <option value="">Sélectionner...</option>
                                            @foreach($unitesMesure as $unite)
                                                <option value="{{ $unite->id }}">
                                                    {{ $unite->code_unite }} - {{ $unite->libelle_unite }}
                        ...
                        {{-- Section Articles --}}
                        <div class="col-12">
                            <div class="article-selector p-3 bg-light rounded">
                                <div class="card border-0 shadow-sm">
                                    <div class="card-body">
                                        ...
                                        <div class="articles-container" style="max-height: 300px; overflow-y: auto;">
                                            @foreach($familles as $famille)
                                                <div class="famille-group mb-3">
                                                    <div class="famille-header d-flex align-items-center justify-content-between bg-light p-2 rounded">
                                                        <div class="d-flex align-items-center">
                                                            <i class="fas fa-tags me-2 text-muted"></i>
                                                            <h6 class="mb-0">{{ $famille->libelle_famille }}</h6>
                                                        </div>
                                                        <div class="form-check">
                                                            <input type="checkbox" class="form-check-input select-all-famille"
                                                                   data-famille="{{ $famille->id }}">
                                                            <label class="form-check-label">Tout sélectionner</label>
                                                        </div>
                                                    </div>
                                                    <div class="article-list ps-3">
                                                        @if($famille->articles->isNotEmpty())
                                                            @foreach($famille->articles->where('statut', true) as $article)
                                                                <div class="article-item"
                                                                     data-search="{{ strtolower($article->code_article . ' ' . $article->designation) }}">
                                                                    <div class="form-check">
                                                                        <input type="checkbox" class="form-check-input article-checkbox"
                                                                               name="article_ids[]" value="{{ $article->id }}"
                                                                               data-famille="{{ $famille->id }}">
                                                                        <label class="form-check-label">
                                                                            <strong>{{ $article->code_article }}</strong>
                                                                            <span class="text-muted ms-2">{{ $article->designation }}</span>
                                                                        </label>
                                                                    </div>
                                                                </div>
                                                            @endforeach
                                                        @else
                                                            <div class="text-muted p-2">Aucun article dans cette famille</div>
                                                        @endif
                                                    </div>
                                                </div>
                                            @endforeach
                                        ...
                                        <div class="selected-count mt-2 text-muted">
                                            <small>0 article(s) sélectionné(s)</small>
                                        </div>
                                    </div>
                                </div>
                            </div>
