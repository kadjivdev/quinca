...
<div class="content">
    @include('pages.achat.requete.partials.header')
    <div class="row g-3 list mt-3">
        <div class="col-12">
            <div class="card border-0 shadow-sm p-3">
                <div class="">
                    <a href="{{route('requetes-frs.index')}}" class="btn btn-sm btn-primary float-right"><i class="bi bi-arrow-left-circle"></i> Retour</a>
                </div>
                <br>
                <form action="{{route('requetes-frs.update',$requete->id)}}" method="POST" id="editRequeteForm" class="needs-validation" novalidate>
                    @csrf
                    @method('PATCH')
                                    ...
                                    <div class="card-body">
                                        <div class="row g-3">
                                            ...
                                            <div class="col-md-6">
                                                ...
                                                <select name="articles[]" multiple class="form-control form-select edit-select2">
                                                    @foreach($articles as $article)
                                                    <option @if(in_array($fournisseur->id,$requete->articles->pluck('id')->toArray())) selected @endif value="{{$article->id}}">{{$article->designation}}</option>
                                                    @endforeach
                                                </select>
