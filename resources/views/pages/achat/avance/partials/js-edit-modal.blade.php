<script>
    // Fonction pour modifier une avance
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    $('#editAcompteForm').attr('action', `${apiUrl}/achat/avances-frs/${id}/update`);

                    // Remplir les champs avec les bons IDs
                    $('#edit_id').val(avance.id);
                    $('#edit_date').val(avance.date);
                    $('#edit_client_id').val(avance.fournisseur.id).trigger('change'); // Correction ici
                    $('#edit_type_paiement').val(avance.type_paiement);
                    $('#edit_montant').val(avance.montant);
                    $('#edit_observation').val(avance.observation);
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    Toast.fire({
                        icon: 'success',
                        title: 'Avance modifiée avec succès'
                    });
    ...
    function updateStats(stats) {
        if (stats) {
            ...
            $('.stat-mois').text(formatNumber(stats.avances_mois));
