<script>
    $(document).ready(function() {
        ...
        $('#depotSelect').on('change', function() {
            ...
            let route = '{{ route("vente.getDepotFactureRevendeur", ":depotId") }}';
        ...
        function chargerLignesFacture(factureId, depotId) {
            ...
            $.ajax({
                url: `${apiUrl}/revendeurs/livraisons/facture/${factureId}/lignes-disponibles`,
                // data: {
                //     depot_id: depotId
                // },
        ...
        $('#addLivraisonForm').on('submit', function(e) {
            ...
            $.ajax({
                url: `${apiUrl}/revendeurs/livraisons`,
                type: 'POST',
                data: formData,
                processData: false,
                contentType: false,
                success: function(response) {
                    if (response.success) {
                        ...
                        window.location.href = `${apiUrl}/revendeurs/livraisons`
                    } else {
                        Toast.fire({
