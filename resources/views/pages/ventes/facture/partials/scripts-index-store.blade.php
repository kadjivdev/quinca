...
const FactureConfig = {
    routes: {
        ...
        getTarifs: `${apiUrl}/vente/factures/articles/:id/tarifs`,
        getUnites: `${apiUrl}/vente/factures/articles/:id/unites'`,
        store: `${apiUrl}/vente/factures`,
        validate: `${apiUrl}/vente/factures/:id/validate`,
        cancel: `${apiUrl}/vente/factures/:id/cancel`
    },
    selectors: {
...
class FactureManager {
    ...
    async handleArticleSelection(index, articleId) {
        ...
        try {
            const [tarifsResponse, unitesResponse] = await Promise.all([
                this.fetchData(this.getRoute('getTarifs', { id: articleId })),
                this.fetchData(this.getRoute('getUnites', { id: articleId }))
            ]);
    ...
    calculateRowTotal(row) {
        try {
            const quantite = parseFloat(row.querySelector(`.${FactureConfig.classes.quantiteInput}`).value) || 0;
    ...
    updateTotals() {
        ...
        document.querySelectorAll('.ligne-facture').forEach(row => {
            const totalLigne = parseFloat(row.querySelector(`.${FactureConfig.classes.totalLigne}`).value.replace(/[^\d.-]/g, '')) || 0;
            const tva = parseFloat(row.querySelector(`.${FactureConfig.classes.tvaInput}`).value) || 0;
        ...
        document.querySelector(FactureConfig.selectors.totalHT).textContent = `${this.formatMoney(totalHT)} FCFA`;
        document.querySelector(FactureConfig.selectors.totalTVA).textContent = `${this.formatMoney(totalTVA)} FCFA`;
        document.querySelector(FactureConfig.selectors.totalTTC).textContent = `${this.formatMoney(totalTTC)} FCFA`;
    }
