<script>
    ...
    class UpdateFactureManager {
        ...
        async handleArticleSelect(e, $line) {
            ...
            try {
                ...
                const [tarifsRes, unitesRes] = await Promise.all([
                    fetch(`${apiUrl}/revendeurs/factures/articles/${articleId}/tarifs`),
                    fetch(`${apiUrl}/revendeurs/factures/articles/${articleId}/unites`)
                ]);
        ...
        async loadUnites(articleId, $line, selectedUniteId = null) {
            try {
                const response = await fetch(`${apiUrl}/revendeurs/factures/articles/${articleId}/unites`);
        ...
        async fillLine($line, data) {
            ...
            $line.find('.taux_remise').val(data.taux_remise);
            $line.find('.depot-input').val(data.depot);
            $line.find('.total-ligne').val(data.montant_ttc);
        ...
        async addLigne(data = null) {
            ...
            articleSelect.select2({
                ...
                width: '100%',
                ajax: {
                    url: `${apiUrl}/revendeurs/factures/api/articles/search`,
                    dataType: 'json',
                    delay: 250,
                    data: (params) => ({
                        q: params.term,
        ...
        async fillLigne($line, data) {
            ...
            // Charger les unités
            if (data.article_id) {
                const unitesRes = await fetch(`${apiUrl}/revendeurs/factures/articles/${data.article_id}/unites`);
                const unites = await unitesRes.json();
    ...
    function editFactures(id) {
        ...
        fetch(`${apiUrl}/revendeurs/factures/${id}`, {
                headers: {
                    'X-Requested-With': 'XMLHttpRequest',
