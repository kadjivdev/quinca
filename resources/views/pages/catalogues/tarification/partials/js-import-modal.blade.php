<script>
    // Gestion du formulaire d'import des articles
    $('#__importArticleForm').on('submit', function(e) {
        e.preventDefault();

        const formData = new FormData(this);
        const submitBtn = $(this).find('button[type="submit"]');
        const modal = $('#importArticleModal');
        ...
        $.ajax({
            ...
            complete: function() {
                ...
                $('#importArticleForm')[0].reset();
            }
        });
    });

    // Réinitialisation du formulaire à la fermeture du modal
    $('#__importArticleModal').on('hidden.bs.modal', function() {
        $('#importArticleForm')[0].reset();
        const submitBtn = $(this).find('button[type="submit"]');
        submitBtn.prop('disabled', false);
        submitBtn.html('<i class="fas fa-file-import me-2"></i>Importer');
