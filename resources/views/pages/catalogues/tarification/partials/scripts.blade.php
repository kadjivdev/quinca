<script>
        ...
        window.editTarification = async function(id) {
            try {
                ...
                if (result.success) {
                    ...
                    if (unite_mesures.length > 0) {
                        unite_mesures.forEach(mesure => {
                            uniteMesureHtml += `<option value="${mesure.id}" ${tarification.unite_mesure_id==mesure.id? 'selected':null}>${mesure.text}</option>`
                        });
                    }
    ...
    async function showAddTarificationModal(articleId, typeTarif) {
        ...
        try {
            const response = await fetch(`/catalogue/articles/${articleId}/edit`, {
                headers: {
                    'Accept': 'application/json',
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
                }
            ...
            document.getElementById('add_type_tarif_id').innerHTML = `<option value="${typeTarif.id}" >
            ...
            if (uniteMesures.length > 0) {
                uniteMesures.forEach(mesure => {
                    uniteMesureHtml += `<option value="${mesure.id}">${mesure.text}</option>`
                });
            }
