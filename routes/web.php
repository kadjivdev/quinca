...
Route::get("/debug", function () {
    ...
    // return Article::firstWhere("code_article","ART-1545");
    ...
    // $lignesBonLivraison = LigneBonLivraisonFournisseur::where("article_id", 1544)
    //     ->with("bonLivraison", "uniteMesure")
    ...
    // $backLignes = LigneMarchandise::with("marchandBack", "article")
    //     ->whereHas("marchandBack", function ($query) {
    //         $query->where("depot_id", 4)
    //             ->whereNotNull("validated_by");
    //     })
    //     // ->where("article_id", 383)
    //     ->get();

    // return $backLignes->pluck("article.code_article");
    ...
    // $lignesVentes = LigneFacture::where("article_id", 1546)
    //     ->where("depot", 4)
    //     ->with("article", "factureClient", "facturedepot", "uuniteVente")
    ...
    // return $lignesVentes->sum("quantite_base");
...
Route::middleware('auth')->group(function () {
    ...
    Route::prefix('achat')->middleware(['auth'])->group(function () {
        ...
        /** GESTION DES AVANCES FOURNISSEUR */
        Route::prefix('avances-frs')->group(function () {
            // Liste des avances
            Route::get('/', [AvanceController::class, 'index'])
                ->name('achat.avances.index');

            // Rafraîchissement de la liste des avances (pour AJAX)
            Route::get('/refresh-list', [AvanceController::class, 'refreshList'])
                ->name('achat.avances.refresh-list');

            // Création d'un nouvel avance
            Route::post('/', [AvanceController::class, 'store'])
                ->name('achat.avances.store');

            // Update d'un avance
            Route::patch('/{avance}/update', [AvanceController::class, 'update'])
                ->name('achat.avances.update')
                ->where('avance', '[0-9]+');

            // Voir les détails d'un avance
            Route::get('/{avance}', [AvanceController::class, 'show'])
                ->name('achat.avances.show')
                ->where('avance', '[0-9]+');

            // Supprimer un avance
            Route::delete('/{avance}', [AvanceController::class, 'destroy'])
                ->name('achat.avances.destroy')
                ->where('avance', '[0-9]+');

            // Valider avance
            Route::post('{avance}/validate', [AvanceController::class, 'validate_acompte'])
                ->name('avances.validate');

            //Rejeter avance
            Route::post('{avance}/reject', [AvanceController::class, 'reject'])
                ->name('avances.reject');
        });
    });
