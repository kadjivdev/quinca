...
class AccompteFournisseurController extends Controller
{
    /**
    ...
    public function index(Request $request)
    {
        ...
        if ($request->filled('fournisseur_id')) {
            $acomptes->where('fournisseur_id', $request->fournisseur_id);
        }
        ...
        // Statistiques
        $stats = [
            'total_acomptes' => AccompteFournisseur::count(),
            'total_montant' => AccompteFournisseur::sum('montant'),
        ...
        return view('pages.achat.acompte.index', compact(
            'acomptes',
            'stats',
            'fournisseurs',
            'date'
    ...
    public function store(Request $request)
    {
        try {
            ...
            $validated = $request->validate(AccompteFournisseur::rules(), [
            ...
            $validated['statut'] = AccompteFournisseur::STATUT_EN_ATTENTE;

            $acompte = new AccompteFournisseur();
            ...
            return response()->json([
                ...
                'data' => [
                    'acompte' => $acompte->load(['fournisseur', 'createdBy'])
                ]
            ]);
     ...
     */
    public function show(Request $request, AccompteFournisseur $acompte)
    {
        if (!$request->ajax()) {
            return response()->json(['error' => 'Requête non autorisée'], 403);
        }

        $acompte->load(['fournisseur', 'createdBy']);
        ...
        return response()->json([
            ...
            'data' => [
                'acompte' => [
                    ...
                    'fournisseur' => [
                        'id' => $acompte->fournisseur->id,
                        'code_fournisseur' => $acompte->fournisseur->code_fournisseur,
                        'raison_sociale' => $acompte->fournisseur->raison_sociale
                    ],
     ...
     * Supprime un acompte
     */
    public function destroy(Request $request, AccompteFournisseur $acompte)
    {
        if (!$request->ajax()) {
            return response()->json(['error' => 'Requête non autorisée'], 403);
        }
        ...
        try {
            ...
            Log::info('Acompte supprimé:', [
                ...
                'reference' => $acompte->reference,
                'fournisseur_id' => $acompte->fournisseur_id,
                'montant' => $acompte->montant,
                'user_id' => auth()->id()
            ]);
     ...
     */
    public function update(Request $request, AccompteFournisseur $acompte)
    {
        try {
            // Validation des données
            $validated = $request->validate(AccompteFournisseur::rules(), [
     ...
     * Valider un acompte
     */
    public function validate_acompte(Request $request, AccompteFournisseur $acompte)
    {
        try {
            if (!$request->ajax()) {
                return response()->json(['error' => 'Requête non autorisée'], 403);
            ...
            return response()->json([
                ...
                'data' => [
                    'acompte' => $acompte->load(['fournisseur', 'createdBy', 'validatedBy'])
                ]
            ]);
        } catch (Exception $e) {
            DB::rollBack();
     ...
     * Rejeter un acompte
     */
    public function reject(Request $request, AccompteFournisseur $acompte)
    {
        try {
            if (!$request->ajax()) {
                return response()->json(['error' => 'Requête non autorisée'], 403);
            ...
            // Rejeter l'acompte
            $acompte->update([
                'statut' => AccompteFournisseur::STATUT_REJETE,
