...
class FactureManager {
    ...
    async initArticleSelect(row, index) {
        ...
        // this.initiatedArticles.push(articleSelect)
            ...
            success: function (response) {
                if (response.results) {
                    ...
                    response.results.forEach(function (data) {
                        let rowOption = `<option value="${data.id}">
                                                <div class="select2-result-article">
                                                    <div class="select2-result-article__code">${data.code_article || ''}</div>
                                                    <div class="select2-result-article__title">${data.text || ''}</div>
                                                    <div class="select2-result-article__stock">Stock: ${data.depot.libelle_depot} (${data.stock})</div>
                                                </div>
                                        </option>`
                        articleSelect.append(rowOption)
                    })
        ...
        // console.log(this.initiatedArticles)
    }
